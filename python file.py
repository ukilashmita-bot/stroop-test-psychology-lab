...
let expName = 'Stroop 2';  // from the Builder filename that created this script
let expInfo = {
...
psychoJS.start({
  ...
  resources: [
    // resources:
    {'name': 'stroop 1.xlsx', 'path': 'Documents/stroop 1.xlsx'},
  ]
});
...
function insRoutineBegin(snapshot) {
  return async function () {
    ...
    insClock.reset(routineTimer.getTime());
    routineTimer.add(1.000000);
    insMaxDurationReached = false;
    // update component parameters for each repeat
    text_2.setText('Welcome');
    psychoJS.experiment.addData('ins.started', globalClock.getTime());
    insMaxDuration = null
    // keep track of which components have finished
    insComponents = [];
    insComponents.push(text_2);
...
function insRoutineEnd(snapshot) {
  return async function () {
    ...
    if (routineForceEnded) {
        routineTimer.reset();} else if (insMaxDurationReached) {
        insClock.add(insMaxDuration);
    } else {
        insClock.add(1.000000);
    }
...
function trialsLoopBegin(trialsLoopScheduler, snapshot) {
  return async function() {
    ...
    trials = new TrialHandler({
      ...
      trialList: 'C:/Users/Ashmita/Documents/stroop 1.xlsx',
  ...
  return async function () {
    ...
    if (key_resp.status === PsychoJS.Status.STARTED) {
      let theseKeys = key_resp.getKeys({
        keyList: typeof ['r','b'] === 'string' ? [['r','b']] : ['r','b'], 
        waitRelease: false
      });
...
function trialRoutineEnd(snapshot) {
  return async function () {
    ...
    if (key_resp.keys === undefined) {
      if (['None','none',undefined].includes(correct_res)) {
